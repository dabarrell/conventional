  ...
  namespace :spec do
    ...
    task :verify_coverage do
      if suite.current_coverage.to_i < suite.coverage_threshold
        puts "Coverage too low. Current: #{suite.current_coverage}%; Threshold: #{suite.coverage_threshold}%"
        exit 1
      else
        puts "Coverage above threshold. Current: #{suite.current_coverage}%; Threshold: #{suite.coverage_threshold}%"
      end
    end
  end
